...
test("tool permissions support exact and prefix rules with deny precedence", () => {
  const policy = {
  ...
  assert.equal(toolPermissionDecision(policy, "mcp__agent_project_console__delete_project"), "deny");
