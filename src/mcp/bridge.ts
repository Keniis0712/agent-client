...
const ORCHESTRATOR_TOOLS: Tool[] = [
  ...
  { name: "list_execution_targets", description: "List online target devices and the independent Codex/Claude agent platforms available on each device.", inputSchema: objectSchema() },
  ...
  { name: "start_project_agent", description: "Start a project agent on a device. The Console resolves the project's execution directory from its local path.", inputSchema: objectSchema({ project_id: { type: "string" }, client_id: { type: "string" }, device_id: { type: "string" }, agent: { enum: ["codex", "claude"] }, message: { type: "string" }, runtime_profile_id: { type: "string" }, permission_policy: { type: "object" } }, ["project_id", "client_id", "device_id", "agent", "message"]) },
