...
const ORCHESTRATOR_TOOLS: Tool[] = [
  ...
  { name: "resolve_project_approval", description: "Queue a decision for an approval waiting in a project agent session. A successful response confirms transport only; wait for the project agent's semantic result before claiming success.", inputSchema: objectSchema({ project_run_id: { type: "string" }, approval_id: { type: "string" }, decision: objectSchema({ type: { enum: ["allow_once", "allow_session", "allow_with_changes", "deny", "cancel"] }, updatedInput: { type: "object" }, message: { type: "string" } }, ["type"]) }, ["project_run_id", "approval_id", "decision"]) },
